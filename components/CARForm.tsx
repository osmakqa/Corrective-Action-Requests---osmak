        ...
        <div className={`rounded-xl border overflow-hidden transition-all duration-300 ${canRespond ? 'bg-white border-blue-400 ring-2 ring-blue-200 shadow-xl' : 'bg-gray-50 border-gray-200 opacity-80'}`}>
           ...
           <div className="p-6">
                 ...
                 <div className="flex justify-between items-center mb-2">
                    ...
                    {canRespond && (
                      <button 
                        ...
                        className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-xl font-bold flex items-center gap-2 shadow-md transition-all active:scale-95 disabled:opacity-50"
                      >
                        {isRemedialAiLoading ? <Loader2 size={18} className="animate-spin" /> : <Bot size={18} />}
                        AI Assist
                      </button>
                    )}
                 </div>
                 
                 {remedialSuggestions.length > 0 && canRespond && (
                   <div className="mb-3 p-3 bg-purple-50 rounded-lg border border-purple-100 animate-in fade-in slide-in-from-top-2 duration-300">
                      <p className="text-[10px] font-bold text-purple-400 uppercase mb-2">AI Suggested Corrections (Click to add):</p>
                      <div className="flex flex-wrap gap-2">
                        {remedialSuggestions.map((s, i) => (
                          <button 
                            key={i} 
                            onClick={() => selectRemedialSuggestion(s)}
                            className="text-left text-xs bg-white hover:bg-purple-600 hover:text-white text-purple-700 px-3 py-1.5 rounded-full border border-purple-200 shadow-sm transition-all"
                          >
                            + {s}
                          </button>
                        ))}
              ...
              <div className="mb-8">
                    ...
                    {canRespond && (
                      <div className="flex flex-col items-end">
                        <button 
                          ...
                          className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-xl font-bold flex items-center gap-2 shadow-md transition-all active:scale-95 disabled:bg-gray-300 disabled:shadow-none disabled:cursor-not-allowed"
                          title={car.rootCauses.length === 0 ? "Identify root causes first to use AI suggestions" : ""}
                        >
                          {isCorrectiveAiLoading ? <Loader2 size={18} className="animate-spin" /> : <Bot size={18} />}
                          AI Assist
                        </button>
                 ...
                 {correctiveSuggestions.length > 0 && canRespond && (
                   <div className="mb-3 p-3 bg-purple-50 rounded-lg border border-purple-100 animate-in fade-in slide-in-from-top-2 duration-300">
                      <p className="text-[10px] font-bold text-purple-400 uppercase mb-2">AI Suggested long-term actions (Click to populate):</p>
                      <div className="flex flex-wrap gap-2">
                        {correctiveSuggestions.map((s, i) => (
                          <button 
                            key={i} 
                            onClick={() => selectCorrectiveSuggestion(s)}
                            className="text-left text-xs bg-white hover:bg-purple-600 hover:text-white text-purple-700 px-3 py-1.5 rounded-full border border-purple-200 shadow-sm transition-all"
                          >
                            {s}
                          </button>
